...
def _to_npu_mx_scale_layout(scale: torch.Tensor) -> torch.Tensor:
    """Convert [M, K/32] scale bytes to the ACLNN [M, K/64, 2] layout."""
    scale_bytes = _scale_bytes(scale)
    assert scale_bytes.shape[-1] % 2 == 0
    return scale_bytes.reshape(scale_bytes.shape[0], -1, 2)
...
def test_mxfp8_scale_semantics_and_quant_matmul(m: int, n: int, k: int) -> None:
    ...
    actual = torch_npu.npu_quant_matmul(
        ...
        pertoken_scale=_to_npu_mx_scale_layout(x_npu_scale),
    ...
    actual_vllm_x = torch_npu.npu_quant_matmul(
        ...
        weight_scale_npu,
        scale_dtype=FLOAT8_E8M0FNU_DTYPE,
        pertoken_scale=_to_npu_mx_scale_layout(x_vllm_scale),
        pertoken_scale_dtype=FLOAT8_E8M0FNU_DTYPE,
        output_dtype=torch.bfloat16,
        group_sizes=[1, 1, BLOCK_SIZE],
    )
