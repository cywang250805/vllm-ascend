...
class TestPrepareAndFinalize(unittest.TestCase):
    ...
    @patch("vllm_ascend.ops.fused_moe.prepare_finalize.get_flash_common3_context", return_value=None)
    @patch("vllm_ascend.ops.fused_moe.prepare_finalize._EXTRA_CTX")
    @patch("torch.ops.vllm.maybe_all_gather_and_maybe_unpad", create=True)
    def test_allgather_ep_prepare_falls_back_without_fc3_context(
        self,
        mock_maybe_all_gather_and_maybe_unpad,
        mock_extra_ctx,
        mock_get_flash_common3_context,
    ):
        def mock_gather(tensor, *args):
            return torch.cat([tensor, tensor], dim=0)

        mock_maybe_all_gather_and_maybe_unpad.side_effect = mock_gather
        ...
        prepare_output = layer._prepare_with_ep_group(hidden_states, router_logits)

        self.assertEqual(mock_maybe_all_gather_and_maybe_unpad.call_count, 2)
        self.assertEqual(prepare_output.hidden_states.shape[0], 6)
        self.assertEqual(prepare_output.router_logits.shape[0], 6)
