...
class TestAscendW8A8FusedMoEMethod(TestBase):
    ...
    def test_apply_overlap_gate_falls_back_when_fc3_disabled(
        ...
        hidden_size = self.hidden_size
        layer = torch.nn.Module()
        layer.w13_weight = torch.randint(-8, 8, (self.num_experts, 2 * self.intermediate_size, hidden_size), dtype=torch.int8)
        layer.w2_weight = torch.randint(-8, 8, (self.num_experts, hidden_size, self.intermediate_size), dtype=torch.int8)
        layer.w13_weight_scale_fp32 = torch.ones(self.num_experts, 2 * self.intermediate_size, dtype=torch.float32)
        layer.w2_weight_scale = torch.ones(self.num_experts, hidden_size, dtype=torch.float32)
    ...
    @patch("vllm_ascend.quantization.methods.w8a8_dynamic._EXTRA_CTX")
    @patch("vllm_ascend.quantization.methods.w8a8_dynamic.select_experts")
    def test_apply_overlap_gate_falls_back_without_shared_context(
        self,
        mock_select_experts,
        mock_extra_ctx,
        mock_get_flash_common3_context,
