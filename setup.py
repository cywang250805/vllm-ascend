...
def get_chip_type() -> str:
    try:
        ...
        # Docker build 环境 fallback：优先读环境变量，否则默认 950PR
        chip_type = os.environ.get('VLLM_ASCEND_CHIP_TYPE', 'ascend950pr')
        logging.warning(f"Warning: npu-smi not available ({e}), using chip type: {chip_type}")
        return chip_type
