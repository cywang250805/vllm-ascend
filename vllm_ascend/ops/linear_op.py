...
class SequenceColumnParallelOp(CustomColumnParallelOp):
    def apply_impl(self, input_: torch.Tensor) -> torch.Tensor | tuple[torch.Tensor, Parameter | None]:
        ...
        # Low-precision communication: quantize before AllGather to save bandwidth
        from vllm_ascend.quantization.methods.w8a8_mxfp8 import AscendW8A8MXFP8DynamicLinearMethod
        actual_quant_method = getattr(self.quant_method, 'quant_method', self.quant_method)
        if need_all_gather and isinstance(actual_quant_method, AscendW8A8MXFP8DynamicLinearMethod):
            from vllm_ascend.device.mxfp_compat import FLOAT8_E8M0FNU_DTYPE
            # 1. Quantize locally: [N/TP, H] bf16 → [N/TP, H] fp8 + [N/TP, H/32] scale
            quantized_x, pertoken_scale = torch_npu.npu_dynamic_mx_quant(input_, dst_type=torch.float8_e4m3fn)

            if get_ascend_config().enable_mc2_linear_fusion:
                hcom_name = get_tp_group().device_group._get_backend(torch.device("npu")).get_hccl_comm_name(self.layer.tp_rank)
                output_parallel, _, _ = torch_npu.npu_all_gather_quant_mm(
                    quantized_x,
                    self.layer.weight,
                    hcom_name,
                    self.layer.tp_size,
                    ...
                    x1_scale_dtype=FLOAT8_E8M0FNU_DTYPE,
                    x2_scale_dtype=FLOAT8_E8M0FNU_DTYPE,
                    y_dtype=input_.dtype,
                    group_sizes=[1, 1, actual_quant_method.group_size],
                    bias=bias,
                )
            else:
                # Keep the existing op for non-FlashComm and padded batches so its no-op/unpad behavior is preserved.
                quantized_x = torch.ops.vllm.maybe_all_gather_and_maybe_unpad(quantized_x, label=True)
                pertoken_scale = torch.ops.vllm.maybe_all_gather_and_maybe_unpad(pertoken_scale, label=True)
                output_parallel = torch_npu.npu_quant_matmul(
                    quantized_x,
                    self.layer.weight,
...
class SequenceRowParallelOp(CustomRowParallelOp):
    ...
    def matmul_and_reduce(self, input_parallel: torch.Tensor, bias_: Parameter | None) -> torch.Tensor:
        ...
        # For w8a8 mxfp8 dynamic quant - fuse into npu_quant_mm_reduce_scatter
        elif get_ascend_config().enable_mc2_linear_fusion and mmrs_fusion and (
            isinstance(self.layer.quant_method, AscendLinearMethod)
            ...
            quant_method_impl = self.layer.quant_method.quant_method
            output, _ = torch_npu.npu_quant_mm_reduce_scatter(
                ...
                bias=bias_,
                y_dtype=x.dtype,
                group_sizes=[1, 1, quant_method_impl.group_size],
            )
        else:
            output_parallel = self.layer.quant_method.apply(self.layer, x, bias=bias_)
...
def _get_column_parallel_op(
    ...
    if enable_sp():
        if "shared_expert" in prefix or "shared_mlp" in prefix:
            return None
        sp_column_prefix = [
...
def get_parallel_op(disable_tp, prefix, layer, direct):
    if (
        ...
        or ("shared_mlp" in prefix and shared_expert_dp_enabled())
    ):
        return None, 0, 1
    custom_op: (
        MLPColumnParallelOp
