...
def set_ascend_forward_context(
    ...
    with set_forward_context(**forward_context_kwargs):
        ...
        if is_draft_model:
            # Drafter models (including MoE drafters like MTP) must disable
            # FlashComm V1: the attention block_table / attn_metadata is built
            # for the original token count, but FlashComm V1's AllGather in
            # qkv_proj would inflate the query size, causing a shape mismatch
            # in the attention kernel.
            flash_comm_v1_enabled = False
        elif is_context_moe_model:
        ...
        if is_draft_model:
            flashcomm_v3_enabled = False
        else:
            flashcomm_v3_enabled = (
                get_ascend_config().multistream_overlap_gate
                and moe_comm_type == MoECommType.ALLGATHER
            )
        forward_context.flashcomm_v3_enabled = flashcomm_v3_enabled
