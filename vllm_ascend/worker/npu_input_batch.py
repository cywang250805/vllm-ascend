...
class NPUInputBatch(InputBatch):
    def __init__(
        ...
        # Persistent per-penalty bin stats, maintained incrementally by
        # add/append/remove/swap/condense so the sampler skips per-step bincount.
        # prompt_bin_mask: packed 32-bit "token in prompt" (repetition).
        # output_bin_counts: per-token counts (freq/presence/repetition).
        self.prompt_bin_mask = torch.zeros(
            (max_num_reqs, cdiv(self.vocab_size, 32)),
            dtype=torch.int32,
            device=device,
        )
        self.output_bin_counts = torch.zeros(
            (max_num_reqs, self.vocab_size),
            dtype=torch.int32,
            device=device,
        )
        # Scratch rows for swap_states row exchange.
        self._prompt_bin_mask_tmp_row_gpu = torch.empty(
            (cdiv(self.vocab_size, 32),),
            dtype=self.prompt_bin_mask.dtype,
            device=device,
        )
        self._output_bin_counts_tmp_row_gpu = torch.empty(
            (self.vocab_size,),
            dtype=self.output_bin_counts.dtype,
            device=device,
        )

        self._arange_max_num_reqs = torch.arange(max_num_reqs, device=device)
