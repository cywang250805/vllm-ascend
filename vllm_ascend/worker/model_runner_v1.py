...
from vllm_ascend.ops.kv_cache_fp8 import fp8_per_head_scale_elems_padded
...
class NPUModelRunner(GPUModelRunner):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # PD-disaggregation optimization switch. When enabled AND this worker
        # is a dedicated kv-producer (pure Prefill) node, the per-step
        # broadcast of sampled_token_ids from the last PP rank can be skipped
        # because the token is never consumed locally (requests are handed
        # off to D nodes after the last prefill chunk).
        # Read once at init time; env var is constant for the lifetime of the
        # process and must be identical across all PP ranks to keep the
        # broadcast-skip decision consistent.
        self._cpp_pd_optimize_enabled: bool = envs_ascend.CPP_PD_OPTIMIZE
        ...
        # OPTIMIZATION flag: set by execute path to tell sample_tokens fast-path
        # whether prev_sampled_token_ids broadcast was skipped this step.
        self._broadcast_skipped_this_step: bool = False
        # Companion flag: when `_broadcast_skipped_this_step` is True, tells
        # sample_tokens fast-path whether the skip was caused by the pure
        # kv-producer (Prefill-only) optimization. If True, non-last PP ranks
        # must still build `prev_req_id_to_index` locally so that
        # `_update_states` (spec-decode branch) does not assert. Reset per
        # step together with `_broadcast_skipped_this_step`.
        self._broadcast_skip_due_to_pure_producer: bool = False
    ...
    def _is_pure_prefill_producer_node(self) -> bool:
        """
        Return True iff this worker is configured as a dedicated kv-producer
        (Prefill-only) node under PD-disaggregation and the user has opted
        in to the PD-disaggregation optimization.

        Conditions (all required):
          1. `CPP_PD_OPTIMIZE` env var is set (=1). The user explicitly
             opted in and guarantees this worker is a dedicated P node.
             In PD-colocated deployments the flag MUST stay 0.
          2. This worker is configured as a kv-producer.

        NOTE: This is a static / config-level predicate. It does NOT inspect
        per-step scheduler_output. The per-step safety check (that this step
        really is prefill-only) lives in
        `_can_skip_broadcast_for_pure_producer`.
        """
        return self._cpp_pd_optimize_enabled and self.is_kv_producer

    def _can_skip_broadcast_for_pure_producer(
        self, scheduler_output: "SchedulerOutput"
    ) -> bool:
        """
        Return True iff it is SAFE to skip `_pp_broadcast_prev_sampled_token_ids`
        this step under the pure-kv-producer optimization.

        Condition: `_is_pure_prefill_producer_node()` (opt-in + kv-producer role).

        This is a config-level gate; the user opts in via `CPP_PD_OPTIMIZE=1`
        to assert this worker is a dedicated P node under PD-disaggregation
        and that requests are handed off to D nodes after the last prefill
        chunk. We deliberately do NOT inspect per-step `num_computed_tokens`
        here because:
          (a) Under spec decode, `num_computed_tokens` may include
              accepted draft tokens, making a naive comparison against
              `num_prompt_tokens` incorrectly classify a last-prefill
              chunk as "decode" and fall back to the broadcast path,
              which then fails `_pp_broadcast_prev_sampled_token_ids`'s
              `[num_reqs, 1]` shape assertion (the original sampler
              output is `[num_reqs, 1 + num_spec_tokens]`).
          (b) Even in the unlikely case a request truly continues to
              decode locally on a P node, the worst-case after a skip is
              that `_prepare_input_ids` falls back to the scheduler-sent
              path (`prev_sampled_token_ids is None` branch), which is
              slower but correct. Not skipping, by contrast, hits the
              shape assertion and crashes.

        The decision is deterministic from env var + kv config and must be
        identical across all PP ranks.
        """
        # `scheduler_output` is kept in the signature for forward compatibility
        # with future per-step gating heuristics.
        del scheduler_output
        return self._is_pure_prefill_producer_node()

    def _build_prev_req_id_to_index_local_only(self) -> None:
        """
        Build `input_batch.prev_req_id_to_index` locally WITHOUT performing
        the PP broadcast recv of `prev_sampled_token_ids`.

        This mirrors the non-recv portion of
        `_pp_receive_prev_sampled_token_ids_to_input_batch` so that
        `_update_states` (which asserts `prev_req_id_to_index is not None`
        in the spec-decode / async-scheduling branch) still works when we
        skip the broadcast on a pure kv-producer node.

        Side-effects:
          - `prev_req_id_to_index` populated (excluding discarded reqs)
          - Each live req's `output_token_ids` gets a placeholder `-1`
            appended, to keep the per-request cached length consistent
            with what the scheduler/engine expect in async scheduling.
          - Every tracked `req_state.prev_num_draft_len` is reset to 0
            to BYPASS the spec-decode rejection calculation in
            `_update_states`. Without this, the rejection branch would
            dereference `valid_sampled_token_count[prev_req_index]`
            which resolves to an empty list (because
            `_get_valid_sampled_token_count` returns `[]` when
            `prev_sampled_token_ids` is None) and raise IndexError.
            On a pure-P node the sampled token is never consumed as
            input_ids in a subsequent local decode step (requests are
            handed off to D nodes), so skipping rejection accounting
            locally is safe: any stale `prev_num_draft_len` carried
            over from a prior step is re-populated by the scheduler
            in a future step where it is actually needed.

        NOTE: `prev_sampled_token_ids` is deliberately left as None. On a
        pure-P node the sampled token is never consumed as input_ids in a
        subsequent local decode step.
        """
        ...
        for i, req_id in enumerate(self.input_batch.req_ids):
            ...
            # length by appending a placeholder (-1) token id, matching the
            # behavior of _pp_receive_prev_sampled_token_ids_to_input_batch.
            if (req_state := self.requests.get(req_id)) is not None:
                req_state.output_token_ids.append(-1)
        self.input_batch.prev_req_id_to_index = prev_req_id_to_index

        # Reset prev_num_draft_len for every currently-tracked request so
        # the spec-decode rejection branch in `_update_states` is skipped.
    ...
    def _prepare_inputs(
        ...
        if _debug_seq_lens_enabled():
            print(
                f"[SEQ_PREP] t={time.time():.6f} rank={_debug_rank()} "
                f"step={getattr(self, '_debug_step_id', -1)} "
                f"num_reqs={num_reqs} "
                f"req_ids={list(self.input_batch.req_ids[:num_reqs])} "
                f"seq_cpu={self.seq_lens.cpu[:num_reqs].tolist()} "
                f"seq_np={self.seq_lens.np[:num_reqs].tolist()} "
                f"cpu_ptr={self.seq_lens.cpu.data_ptr()} "
                f"gpu_ptr={self.seq_lens.gpu.data_ptr()}",
                flush=True,
            )
        ...
        # Mirror base class: populate the boolean discard_request_mask buffer used
        # by _pp_receive_prev_sampled_token_ids_to_input_batch on non-last PP ranks
        # to exclude mid-prefill-chunk requests from prev_req_id_to_index. Without
        # this, the next step _prepare_input_ids fast-path would scatter the
        # previous step dummy sampled tokens onto those requests last prompt
        # token slots and corrupt prefill (precision issue under PP+async sched).
    ...
    def execute_model(
        ...
        self._debug_step_id = getattr(self, "_debug_step_id", 0) + 1
        ...
        with record_function_or_nullcontext("prepare input"):
            with self.synchronize_input_prep():
                ...
                # OPTIMIZATION: compute broadcast-skip flag here AFTER
                # _update_states so self.requests is populated for the
                # newly-scheduled requests. ALL PP ranks compute identical
                # decision based on the same scheduler_output.
                #
                # Two independent reasons to skip the broadcast:
                #   (a) No request in this step emits a real next token
                #       (all partial prefill chunks); the sampled dummy
                #       values are never consumed by the next step.
                #   (b) This worker is a dedicated kv-producer (pure Prefill)
                #       node under PD-disaggregation (CPP_PD_OPTIMIZE=1) AND
                #       every scheduled request this step is still in
                #       prefill (safety check against mis-routed decode
                #       requests). Requests leave this worker after the
                #       last prefill chunk, so the broadcasted token is
                #       never consumed locally.
                # Both (a) and (b) are deterministic functions of config /
                # scheduler_output and are therefore identical across all
                # PP ranks, which is required for consistent skip/recv.
                if self.use_async_scheduling and get_pp_group().world_size > 1:
                    skip_due_to_no_real_token = (
                        not self._any_request_emits_real_token(scheduler_output)
                    )
                    skip_due_to_pure_producer = (
                        self._can_skip_broadcast_for_pure_producer(scheduler_output)
                    )
                    self._broadcast_skipped_this_step = (
                        skip_due_to_no_real_token or skip_due_to_pure_producer
                    )
                    # Remember why we skipped: pure-producer skip needs to
                    # still build `prev_req_id_to_index` locally on non-last
                    # PP ranks to satisfy the spec-decode assertion in
                    # `_update_states`.
                    self._broadcast_skip_due_to_pure_producer = (
                        self._broadcast_skipped_this_step
                        and skip_due_to_pure_producer
                        and not skip_due_to_no_real_token
                    )
                else:
                    self._broadcast_skipped_this_step = False
                    self._broadcast_skip_due_to_pure_producer = False
    ...
    def sample_tokens(
        ...
        if self.execute_model_state is None:
            ...
            # OPTIMIZATION: skip recv when broadcast was skipped (flag set
            # in execute_model on this rank, identical to last rank's).
            _skip = getattr(self, "_broadcast_skipped_this_step", False)
            if (
                self.use_async_scheduling
                ...
                # There are two reasons we may skip the recv:
                #   (1) All requests this step are partial prefill chunks
                #       that do not emit a real token (middle-chunk case).
                #       Here it is safe to clear both fields because
                #       downstream spec-decode / _update_states paths check
                #       `prev_num_draft_len` which is still 0.
                #   (2) This worker is a pure kv-producer (Prefill-only)
                #       node under PD-disaggregation. Requests may have
                #       `prev_num_draft_len > 0` (spec decode on last
                #       prefill chunk), which makes `_update_states`
                #       assert `prev_req_id_to_index is not None`.
                #       We still skip the actual broadcast recv (no
                #       communication, no `prev_sampled_token_ids`
                #       tensor), but rebuild the local-only mapping so
                #       that assert holds and spec-decode bookkeeping
                #       stays consistent. The sampled token itself is
                #       never consumed locally on a pure-P node because
                #       requests are handed off to D nodes after the
                #       last prefill chunk, so leaving
                #       `prev_sampled_token_ids = None` is correct.
                self.input_batch.prev_sampled_token_ids = None
                if self._broadcast_skip_due_to_pure_producer:
                    self._build_prev_req_id_to_index_local_only()
                else:
        ...
        if self.use_async_scheduling:
            ...
            if pp.world_size > 1 and pp.is_last_rank:
                # OPTIMIZATION: skip broadcast for intermediate prefill chunks.
                # Flag is computed in execute_model() and identical across PP
                # ranks, so non-last ranks will skip recv consistently.
                if not self._broadcast_skipped_this_step:
                    self._pp_broadcast_prev_sampled_token_ids(sampler_output.sampled_token_ids)

        # Reset the per-step skip flag for the next iteration
        # (last rank main path).
        self._broadcast_skipped_this_step = False
        self._broadcast_skip_due_to_pure_producer = False
    ...
    def _build_attention_metadata(
        ...
        cm_base = AscendCommonAttentionMetadata(
            ...
            debug_step_id=getattr(self, "_debug_step_id", -1),
        )
    ...
    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        # NOTE: `self.drafter` is only created on the last PP rank (see
        # `_set_up_drafter`). On non-last PP ranks it stays None, so we must
        # guard the isinstance assertion to avoid AssertionError.
        ...
        if has_kv_transfer_group():
            ...
            cross_layers_kv_cache = getattr(self, "cross_layers_kv_cache", None)
            if cross_layers_kv_cache is not None:
                cross_layers_attn_backend = getattr(
    ...
    def initialize_kv_cache_tensors(self, kv_cache_config: KVCacheConfig) -> dict[str, torch.Tensor]:
        ...
        """
        # Try creating KV caches optimized for kv-connector transfers (cross-layer)
        cache_dtype = self.cache_config.cache_dtype
        if self.use_uniform_kv_cache(self.attn_groups, cache_dtype):
            # current is 64
            attn_backend_cls = self.attn_groups[0][0].backend
            kernel_block_sizes = [attn_backend_cls.get_supported_kernel_block_sizes()[0]]
            ...
            # Fallback to per-layer allocation
        ...
        for group in self._kv_cache_spec_attn_group_iterator():
            ...
            for layer_name in group.layer_names:
                ...
                if isinstance(current_kv_cache_spec, AttentionSpec):
                    ...
                    cache_dtype_str = self.vllm_config.cache_config.cache_dtype
                    use_fp8_kv_cache = cache_dtype_str in ("fp8", "fp8_e4m3")
                    if not isinstance(current_kv_cache_spec, MLAAttentionSpec):
                        k_shape = kv_cache_shape[1:]
                        if hasattr(current_kv_cache_spec, "head_size_v") and not use_fp8_kv_cache:
                    ...
                    if use_fp8_kv_cache and not isinstance(current_kv_cache_spec, MLAAttentionSpec):
                    ...
                    if use_fp8_kv_cache and not isinstance(current_kv_cache_spec, MLAAttentionSpec):
                        ...
                        k_cache = raw_k_tensor.view(k_cache_dtype).view(k_shape_physical).permute(*inv_order_4d)
                        v_cache = raw_v_tensor.view(v_cache_dtype).view(v_shape_physical).permute(*inv_order_4d)
                    else:
        ...
        for layer_name, attn_module in attn_layers.items():
            if isinstance(attn_module, Attention):
                ...
                if spec := attn_module.get_kv_cache_spec(self.vllm_config):
                    if self.vllm_config.cache_config.cache_dtype in ("fp8", "fp8_e4m3"):
                        head_size_v = getattr(spec, "head_size_v", spec.head_size)
                        assert spec.head_size == head_size_v
                        padded = fp8_per_head_scale_elems_padded(
                        ...
                        page_size_padded = (
                            ...
                            * spec.block_size
                            * spec.num_kv_heads
                            * (spec.head_size + padded)
                            * get_dtype_size(torch.float8_e4m3fn)
                        )
                        spec = replace(spec, page_size_padded=page_size_padded)
