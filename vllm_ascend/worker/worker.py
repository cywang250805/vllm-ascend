...
class NPUWorker(WorkerBase):
    ...
    def execute_model(
        ...
        # When VLLM_PP_SEND_OVERLAP is enabled, we delay the wait of the
        # previous PP isend until *after* this step's forward + isend, so that
        # this forward can overlap with the in-flight send of the previous
        # chunk. We also clone the output tensors before isend to avoid the
        # next forward overwriting the buffer that is still being sent
        # (intermediate_tensors buffer is reused across steps in model_runner).
        pp_send_overlap = bool(int(os.environ.get("VLLM_PP_SEND_OVERLAP", "0")))

        if not pp_send_overlap:
            # Original path: wait previous send at entry.
        ...
        if isinstance(output, (ModelRunnerOutput, AsyncModelRunnerOutput, NoneType)):
            # last PP rank or no-op step: nothing to send. Still need to
            # drain any pending previous send if we are in overlap mode.
            if pp_send_overlap and self._pp_send_work:
                for handle in self._pp_send_work:
                    handle.wait()
                self._pp_send_work = []
            return output
        ...
        if pp_send_overlap:
            # Clone tensors into a dedicated send buffer so the next forward
            # can safely overwrite intermediate_tensors while this send is
            # still in flight on the HCCL stream.
            send_tensors = {k: v.clone() for k, v in output.tensors.items()}
            new_send_work = get_pp_group().isend_tensor_dict(
                send_tensors,
                all_gather_group=all_gather_group,
            )
            # Wait the *previous* step's send (one-step lag), so this step's
            # forward already executed concurrently with that previous send.
            if self._pp_send_work:
                for handle in self._pp_send_work:
                    handle.wait()
            self._pp_send_work = new_send_work
        else:
            # Original path: send directly from output.tensors (which points
            # into the reusable intermediate_tensors buffer).
