...
@register_scheme("FAQuant", "attention")
@register_scheme("FAKQuant", "attention")  # 添加FAKQuant的注册，映射到相同的实现
class AscendFAQuantAttentionMethod:
    ...
    def create_weights(self, layer: torch.nn.Module) -> None:
        ...
        params_dict["fa_q.scale"] = torch.empty((layer.num_heads, 1), dtype=torch.float32)
        params_dict["fa_k.scale"] = torch.empty((layer.num_kv_heads, 1), dtype=torch.float32)
        params_dict["fa_v.scale"] = torch.empty((layer.num_kv_heads, 1), dtype=torch.float32)
        params_dict["fa_q.offset"] = torch.empty((layer.num_heads, 1), dtype=torch.float32)
        params_dict["fa_k.offset"] = torch.empty((layer.num_kv_heads, 1), dtype=torch.float32)
        params_dict["fa_v.offset"] = torch.empty((layer.num_kv_heads, 1), dtype=torch.float32)
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # 检查kv_lora_rank是否有效
        if self.kv_lora_rank <= 0 or self.kv_lora_rank > 1000000:
            return
