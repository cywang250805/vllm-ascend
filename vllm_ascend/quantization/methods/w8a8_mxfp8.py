...
class AscendW8A8MXFP8DynamicFusedMoEMethod(AscendMoEScheme):
    ...
    def apply(
        ...
        fc3_context = get_flash_common3_context() if self.multistream_overlap_gate else None
        fc3_enabled = (
            self.multistream_overlap_gate
            and _EXTRA_CTX.flashcomm_v3_enabled
            and fc3_context is not None
            and fc3_context.shared_experts is not None
        )
        if fc3_enabled:
            assert fc3_context.topk_weights is not None and fc3_context.topk_ids is not None, (
                "FC3 topk cache is empty; expected AscendSharedFusedMoE to "
                "populate it before quant_method.apply() is invoked."
            )
