...
def get_quant_type_for_layer(
    ...
    # Check layer-specific quant_type field
    layer_quant_type_key = prefix + ".quant_type"
    if layer_type == "attention" and layer_quant_type_key in quant_description:
        quant_type = quant_description[layer_quant_type_key]
        # Map QK_FP8_DYNAMIC_V_FP8 to FAKQuant
        if quant_type == "QK_FP8_DYNAMIC_V_FP8":
            return "FAKQuant"
        return quant_type
    # Linear / MoE
...
class AscendModelSlimConfig(QuantizationConfig):
    ...
    def get_quant_method(self, layer: torch.nn.Module, prefix: str) -> Optional["QuantizeMethodBase"]:
        ...
        if isinstance(layer, LinearBase):
            ...
            self.is_fa_quant_layer(prefix) or
            self.is_indexer_quant_layer(prefix) or
            self.is_qk_fp8_v_fp8_layer(prefix)
        ):
    ...
    def is_layer_skipped_ascend(self, prefix: str, fused_mapping: Mapping[str, list[str]] = MappingProxyType({})):
        ...
        # 首先检查是否是新的量化类型，如果是则默认不跳过
        layer_prefix = ".".join(prefix.split(".")[:-1])  # 获取layer前缀
        layer_quant_type_key = layer_prefix + ".quant_type"
        if layer_quant_type_key in self.quant_description and \
           self.quant_description[layer_quant_type_key] == "QK_FP8_DYNAMIC_V_FP8":
            return False
    ...
    def is_qk_fp8_v_fp8_layer(self, prefix):
        """检查是否为QK_FP8_DYNAMIC_V_FP8量化层"""
        if self.enable_qk_fp8_v_fp8_quant:
            layer_id_str = "".join(re.findall(r"\.(\d+)\.", prefix))
            if layer_id_str.isdigit() and int(layer_id_str) in self.qk_fp8_v_fp8_quant_layers:
                return True
        return False
    ...
    def enabling_fa_quant(self, vllm_config, layer_name) -> bool:
        ...
        return bool(is_decode_instance and (
            self.is_fa_quant_layer(layer_name) or
            self.is_qk_fp8_v_fp8_layer(layer_name)
        ))
    ...
    def get_kv_quant_dtype(self, layer_name, cache_dtype, model_config):
        ...
        elif self.enable_qk_fp8_v_fp8_quant and self.is_qk_fp8_v_fp8_layer(layer_name):
            # 对于QK_FP8_DYNAMIC_V_FP8类型，使用FP8量化
    ...
    def _add_kvcache_quant_metadata(self):
        ...
        self.enable_qk_fp8_v_fp8_quant = False
        self.qk_fp8_v_fp8_quant_layers = []
        ...
        # 检查QK_FP8_DYNAMIC_V_FP8量化层
        for key in self.quant_description:
            _id = "".join(re.findall(r"\.(\d+)\.", key))
            if key.endswith(".quant_type") and self.quant_description[key] == "QK_FP8_DYNAMIC_V_FP8":
                self.enable_qk_fp8_v_fp8_quant = True
                if _id.isdigit():
                    self.qk_fp8_v_fp8_quant_layers.append(int(_id))
