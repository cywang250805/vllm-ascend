...
class GlobalTE:
    ...
    def get_transfer_engine(self, hostname: str, protocol: str | None, device_name: str | None):
        if self.transfer_engine is None:
            with self.transfer_engine_lock:
                ...
                if self.transfer_engine is None:
                    ...
                    protocol = protocol if protocol is not None else "rdma"
                    logger.info(
    ...
    def register_buffer(self, ptrs: list[int], sizes: list[int]):
        with self.register_buffer_lock:
            ...
            if self.is_register_buffer:
                return
            logical_id = torch_npu.npu.current_device()
            device_id = current_platform.device_id_to_physical_device_id(logical_id)
            logger.info(f"device_id: {device_id}")
            for ptr, size in zip(ptrs, sizes):
                ret_value = self.transfer_engine.register_memory(ptr, size, f"npu:{device_id}")
                if ret_value != 0:
