...
def refresh_block_size(vllm_config):
    ...
    # FP8 attention PA/FIA operator only supports block_size=128. The kernel
    # block size selection (get_supported_kernel_block_sizes) is only consulted
    # in the hybrid-blocks path, so for the common single-attn-group case we
    # must force the manager block_size to 128 here, regardless of cache_dtype.
    if envs_ascend.VLLM_ASCEND_ENABLE_FP8_ATTN and cache_config.block_size != 128:
        logger.info("Block size is set to 128 because VLLM_ASCEND_ENABLE_FP8_ATTN is enabled.")
        cache_config.block_size = 128
