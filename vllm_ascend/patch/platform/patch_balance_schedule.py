...
from vllm.v1.engine import CachedTokensDetail, EngineCoreEventType, EngineCoreOutputs
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while self.waiting and token_budget > 0:
                ...
                connector_cpu_prefix_cache_hits, connector_kvs_prefix_cache_hits = 0, 0
                ...
                if request.num_computed_tokens == 0:
                    ...
                    if self.connector is not None:
                        ...
                        get_hit_details_fn = getattr(self.connector, "get_hit_details", None)
                        if get_hit_details_fn is not None and "Mock" not in type(get_hit_details_fn).__name__:
                            hit_details = get_hit_details_fn(request.request_id)
                            if hit_details is not None:
                                connector_cpu_prefix_cache_hits = hit_details.cpu_hit_tokens
                                connector_kvs_prefix_cache_hits = hit_details.kvs_hit_tokens
                ...
                if self.connector is not None:
                    ...
                    if self.kv_cache_manager.prefix_cache_stats is not None:
                        self.kv_cache_manager.prefix_cache_stats.record_cpu_kvstore(
                            cpu_hits=connector_cpu_prefix_cache_hits,
                            kvs_hits=connector_kvs_prefix_cache_hits,
                        )
                ...
                if load_kv_async:
                    ...
                    if request.num_cached_tokens < 0:
                        ...
                        request.cached_tokens_detail = CachedTokensDetail(
                            num_gpu_cached_tokens=num_new_local_computed_tokens,
                            num_cpu_cached_tokens=connector_cpu_prefix_cache_hits,
                            num_kvs_cached_tokens=connector_kvs_prefix_cache_hits,
                        )
                    skipped_waiting_requests.prepend_request(request)
