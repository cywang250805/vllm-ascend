...
def _cached_tokens_from_detail(cached_tokens_detail: Any | None) -> int:
    if cached_tokens_detail is None:
        return 0
    return (
        (getattr(cached_tokens_detail, "num_gpu_cached_tokens", 0) or 0)
        + (getattr(cached_tokens_detail, "num_cpu_cached_tokens", 0) or 0)
        + (getattr(cached_tokens_detail, "num_kvs_cached_tokens", 0) or 0)
    )


def _select_cache_usage(
    current_tokens: int | None,
    current_detail: Any | None,
    res,
) -> tuple[int | None, Any | None]:
    res_tokens = getattr(res, "num_cached_tokens", None)
    res_detail = getattr(res, "cached_tokens_detail", None)
    res_detail_tokens = _cached_tokens_from_detail(res_detail)
    res_total = max(res_tokens or 0, res_detail_tokens)

    if current_tokens is None:
        return res_total, res_detail

    current_detail_tokens = _cached_tokens_from_detail(current_detail)
    current_total = max(current_tokens or 0, current_detail_tokens)
    if res_total > current_total:
        return res_total, res_detail
    if current_detail_tokens == 0 and res_detail_tokens > 0:
        return current_tokens, res_detail
    return current_tokens, current_detail


def _make_prompt_tokens_details(
    num_cached_tokens: int | None,
    cached_tokens_detail: Any | None,
):
    gpu = getattr(cached_tokens_detail, "num_gpu_cached_tokens", 0) or 0
    cpu = getattr(cached_tokens_detail, "num_cpu_cached_tokens", 0) or 0
    kvs = getattr(cached_tokens_detail, "num_kvs_cached_tokens", 0) or 0
    detail_total = gpu + cpu + kvs
    total = max(num_cached_tokens or 0, detail_total)

    prompt_details = chat_serving.PromptTokenUsageInfo(
        gpu_cached_tokens=gpu,
        cpu_cached_tokens=cpu,
        kvs_cached_tokens=kvs,
    )
    if detail_total == 0:
        # vLLM's PromptTokenUsageInfo validator recomputes cached_tokens from
        # tier counters. Preserve the total-only value when no tier detail is
        # available from the patched Ascend usage path.
        prompt_details.cached_tokens = total
...
def _make_usage_info(
    ...
    *,
    ...
    cached_tokens_detail: Any | None = None,
    reasoning_tokens: int | None = None,
) -> UsageInfo:
    usage = UsageInfo(
...
OpenAIServingChat._select_cache_usage = staticmethod(_select_cache_usage)
...
def _update_usage_tracking_state(
    ...
    state.num_cached_tokens, state.cached_tokens_detail = _select_cache_usage(
        state.num_cached_tokens,
        state.cached_tokens_detail,
        res,
    )
...
def _make_full_response_usage(
    ...
    return self._make_usage_info(
        ...
        cached_tokens_detail=state.cached_tokens_detail,
