...
env_variables: dict[str, Callable[[], Any]] = {
    ...
    # Whether to enable PD-disaggregation specific optimizations.
    # 0, or not set (default): keep behavior identical to PD-colocated (safe).
    # 1: enable PD-disaggregation optimizations, currently including:
    #    - Skip `_pp_broadcast_prev_sampled_token_ids` on kv-producer (pure P)
    #      nodes, because prefill-only nodes never consume the broadcast result
    #      in a subsequent decode step (requests are handed off to D nodes).
    # NOTE: This flag MUST NOT be enabled on PD-colocated (single instance that
    # runs both prefill and decode) deployments, otherwise decode input_ids on
    # non-last PP ranks will be wrong. Only set to 1 when this worker is a
    # dedicated kv-producer (pure Prefill) node.
    "CPP_PD_OPTIMIZE": lambda: bool(int(os.getenv("CPP_PD_OPTIMIZE", "0"))),
    ...
    # Whether to enable the fp8 attention pass-through path (FIA PA + block_size=128).
    # 0 (default): use the backup plan (KV cache fp8 + bf16 attention compute +
    #              block_size=64, FIA goes through the non-PA operator).
    # 1: enable fp8 attention (fp8 Q/K/V + fp32 scale fed directly into the FIA
    #    PA operator, skipping dequantization).
    "VLLM_ASCEND_ENABLE_FP8_ATTN": lambda: bool(int(os.getenv("VLLM_ASCEND_ENABLE_FP8_ATTN", "0"))),
}
