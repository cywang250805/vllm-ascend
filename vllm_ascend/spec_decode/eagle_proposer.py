...
class MultiLayerEagleProposer(SpecDecodeBaseProposer):
    ...
    def dummy_run(
        ...
        batch_size = max(
            num_input_tokens // (self.num_speculative_tokens + 1), 1
        )  # if not is_profile else self.runner.max_num_reqs
        if is_profile:
            batch_size = min(batch_size, self.runner.max_num_reqs)
        ...
        adjust_input_kwargs = {
            ...
            "common_attn_metadata": AscendCommonAttentionMetadata(
                ...
                seq_lens_cpu=torch.tensor(
                    [num_input_tokens], dtype=torch.int32, device="cpu"
                ),
                num_reqs=1,
                num_actual_tokens=num_input_tokens,
    ...
    def _propose(
        ...
        # logger.info(f"=============================rank {torch.distributed.get_rank()} before adjust_input common_attn_metadata {common_attn_metadata}")
        (
            target_token_ids,
            target_positions,
        ...
        # logger.info(f"=============================rank {torch.distributed.get_rank()} after adjust_input common_attn_metadata {common_attn_metadata}")
        ...
        # logger.info(f"=============================rank {torch.distributed.get_rank()} after fisrt pass common_attn_metadata {common_attn_metadata}")
        assert self.runner is not None
        # update pcp related params
        if self.pcp_size * self.dcp_size > 1:
            assert long_seq_metadata is not None
    ...
    def _run_merged_draft(
        ...
        for spec_step_idx in range(self.layer_num):
            ...
            if self.method == "mtp":  # 对应 if self.enable_multi_layers_mtp:
        ...
        # 剩余的token
        for draft_step in range(self.num_speculative_tokens - self.layer_num):
            # Reset MOE layer index for each draft step iteration
            forward_context = get_forward_context()
            if forward_context is not None:
                forward_context.moe_layer_index = 0
            ...
            hidden_states = hidden_states[:batch_size]
            draft_token_ids = logits.argmax(dim=-1)
            #draft_token_ids_tensor[draft_step set_inputs_first_pass
