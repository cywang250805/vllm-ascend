...
from vllm_ascend.ops.kv_cache_fp8 import (
    fp8_per_head_scale_elems_padded,
    gather_dequantize_fp8_kv_cache,
    view_fp8_cache_as_hnd,
    write_fp8_kv_cache,
)
...
class AscendAttentionBackend(AttentionBackend):
    ...
    def get_supported_kernel_block_sizes() -> list[int]:
        if envs_ascend.VLLM_ASCEND_ENABLE_FP8_ATTN:
            # fp8 attention PA operator requires block_size=128.
            return [128]
        # Backup plan: kernel block size set to 64.
        return [64, 128]
...
class AscendAttentionMetadataBuilder(AttentionMetadataBuilder[AscendMetadata]):
    ...
    def build(
        ...
        if _debug_seq_lens_enabled():
            print(
                f"[SEQ_BUILD] t={time.time():.6f} rank={_debug_rank()} "
                f"step={common_attn_metadata.debug_step_id} "
                f"num_reqs={num_reqs} "
                f"num_actual_tokens={num_actual_tokens} "
                f"seq_device={seq_lens.device} "
                f"seq_ptr={seq_lens.data_ptr()} "
                f"seq={seq_lens.tolist() if seq_lens.device.type == 'cpu' else seq_lens.cpu().tolist()}",
                flush=True,
            )
        ...
        attn_metadata = AscendMetadata(
            ...
            debug_step_id=common_attn_metadata.debug_step_id,
...
class AscendAttentionBackendImpl(AttentionImpl):
    def __init__(
        ...
        **kwargs,
        ...
        self._current_v_scale: torch.Tensor | None = None
        self.use_npu_rope_norm = False
        # FIA fp8 直通：直接把 fp8 的 Q/K/V + fp32 scale 喂给 FIA 算子，
        # 不再把它们反量化回 bf16。由 patch_hunyuan_v3 在 fp8 场景下开启，
        # 并通过 _current_q_scale 透传 query 的 dequant scale（NT layout）。
        self.use_fia_fp8 = False
        self._current_q_scale: torch.Tensor | None = None

    def _get_fp8_logical_block_size(self) -> int:
    ...
    def _get_fia_params(self, key: torch.Tensor, value: torch.Tensor, attn_metadata: AscendMetadata):
        ...
        # chunked prefill.
            ...
            block_table = attn_metadata.block_tables
            actual_seq_lengths_kv = attn_metadata.seq_lens_list
            # When target model pads block_table for its graph capture but
            # the padded rows have seq_lens=0, NPU FIA op will reject the
            # shape mismatch.  Trim block_table to match actual batch size.
            # Only trim at runtime (not during graph capture).
            if not _EXTRA_CTX.capturing and actual_seq_lengths_kv and block_table is not None:
                actual_batch = len(actual_seq_lengths_kv)
                if block_table.shape[0] > actual_batch:
                    block_table = block_table[:actual_batch]
        return key, value, block_size, block_table, actual_seq_lengths_kv


    def _get_query_lens(self, actual_seq_lengths_q: list[int]) -> list[int]:
        prev = 0
        query_lens: list[int] = []
        for seq_len in actual_seq_lengths_q:
            query_lens.append(seq_len - prev)
            prev = seq_len
        return query_lens


    def _get_fia_fp8_params(self, attn_metadata: AscendMetadata):
        """为 FIA fp8 直通取参。

        从 fp8 paged cache 折出 BnNBsD 的 fp8 K/V 数据，以及打包在 cache
        padding 里的 fp32 K-scale cache（BnNBs layout，即算子要求的
        keyAntiquantScale）。仅 PA 场景使用。
        """

        assert self.key_cache is not None
        assert self.value_cache is not None
        block_size = self._get_fp8_logical_block_size()
        key_cache_fp8 = (
            self.key_cache.view(torch.float8_e4m3fn)
            if self.key_cache.dtype != torch.float8_e4m3fn
            else self.key_cache
        )
        value_cache_fp8 = (
            self.value_cache.view(torch.float8_e4m3fn)
            if self.value_cache.dtype != torch.float8_e4m3fn
            else self.value_cache
        )
        k_data, k_scale_cache = view_fp8_cache_as_hnd(
            key_cache_fp8, block_size, self.head_size
        )

        v_data, _ = view_fp8_cache_as_hnd(
            value_cache_fp8, block_size, self.head_size
        )
    ...
    def _forward_fp8_prefill_with_context(
        self,
        query: torch.Tensor,
        key: torch.Tensor,
        value: torch.Tensor,
        ...
        """Unified fp8 KV cache attention path: gather + dequant + non-PA FIA."""
        if (
            not self.use_fp8_kv_cache
        ...
        query_lens = self._get_query_lens(attn_metadata.actual_seq_lengths_q)
        num_tokens = attn_metadata.actual_seq_lengths_q[-1]
        query = query[:num_tokens].to(torch.bfloat16)

        block_table = attn_metadata.block_tables
        if block_table is None:
            return None
        if block_table.shape[0] > len(query_lens):
            block_table = block_table[: len(query_lens)]

        block_size = self._get_fp8_logical_block_size()
        # 统一从 cache gather + dequant 全部 KV（融合算子已写入 cache）
        all_kv_lens = attn_metadata.seq_lens_list
        concat_key, concat_value = gather_dequantize_fp8_kv_cache(
            self.key_cache,
            self.value_cache,
            self._current_v_scale,
            block_table,
            all_kv_lens,
            block_size,
            self.head_size,
        ...
        for kv_len in all_kv_lens:
            kv_end += kv_len
            actual_seq_lengths_kv.append(kv_end)
    ...
    def forward_fused_infer_attention(
        ...
        #  Merged C8 Path.
        fp8_prefill_output = self._forward_fp8_prefill_with_context(
            query, key, value, attn_metadata, output
        )
        if fp8_prefill_output is not None:
            return fp8_prefill_output

        if self.use_fp8_kv_cache and self.use_fia_fp8:
            key, value, k_scale_cache, block_size, block_table, actual_seq_lengths_kv = self._get_fia_fp8_params(attn_metadata)
        else:
            # C16 path;
            key, value, block_size, block_table, actual_seq_lengths_kv = self._get_fia_params(key, value, attn_metadata)
        num_tokens = attn_metadata.actual_seq_lengths_q[-1]
        ...
        if self.sinks is not None:
        ...
        elif self.use_fia_fp8:
            # q_query: TND -> NTD
            query = query.permute(1, 0, 2).contiguous()
            q_scale = self._current_q_scale[:num_tokens].transpose(0, 1).contiguous()
            p_scale = torch.ones(1, dtype=torch.float32, device=query.device)
            v_scale_cache = self._current_v_scale.to(torch.float32).squeeze(dim=1)
            k_scale_cache_sq = k_scale_cache.squeeze(dim=-1)
            # TODO: fp8 atten need to slice block_table
            import math
            block_table = block_table[:,:math.ceil(max(actual_seq_lengths_kv)/128)]
            attn_output, _ = torch_npu.npu_fused_infer_attention_score_v2(
                query,
                key,
                value,
                ...
                dequant_scale_key=k_scale_cache_sq,
                dequant_scale_value=v_scale_cache,
                block_table=block_table,
                block_size=block_size,
                num_query_heads=self.num_heads,
                num_key_value_heads=self.num_kv_heads,
                ...
                key_quant_mode=3,  # per-token-head
                value_quant_mode=2,  # per-tensor-head
            ...
            output[:num_tokens] = attn_output[:num_tokens].to(output.dtype)
        else:
            attn_output, _ = torch_npu.npu_fused_infer_attention_score(
                query=query,
                key=key,
    ...
    def reshape_and_cache(
        ...
        if len(kv_cache) > 1:
            ...
            if self.use_fp8_kv_cache and not encoder_decoder:
                assert self._current_v_scale is not None
                write_fp8_kv_cache(
                    cache_key,
                    cache_value,
                    self.key_cache,
                    ...
                    cache_slots,
                    self._current_v_scale,
                    self._get_fp8_logical_block_size(),
                    self.head_size,
                )
            else:
    ...
    def forward(
        ...
        if self.use_fp8_kv_cache:
            fa_v = getattr(layer, "fa_v", None)
            self._current_v_scale = (
                fa_v.scale if fa_v is not None and hasattr(fa_v, "scale")
                else layer._v_scale
            )
        else:
            self._current_v_scale = None
            assert layer._k_scale_float == 1.0 and layer._v_scale_float == 1.0
        num_tokens = query.shape[0]
        if attn_metadata is None:
            if output.dtype in (torch.float8_e4m3fn, torch.float8_e5m2):
                output.view(torch.int8).fill_(0)
